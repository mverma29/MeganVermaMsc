...
``` {r}
owid <- read_csv("/Users/meganverma/Desktop/R/Msc_project/owid_covid.csv") # replace NULL with file name
```
...
``` {r}
iso_cases <- tibble(owid_latest$iso_code, owid_latest$total_cases_per_million) 
world_with_cases <- merge (x=iso_cases, y=world, by.x= "owid_latest$iso_code", by.y = "iso_a3")

world_with_cases <- rename (world_with_cases, iso_code ="owid_latest$iso_code" )
world_with_cases <- rename (world_with_cases, total_cases_per_million ="owid_latest$total_cases_per_million" )
...
# plot this 
ggplot(data = world_with_cases$geometry) +  #world map geometry (polygons)
  geom_sf(aes(fill= world_with_cases$log_total_cases)) + #color map w/ cont. values of LOG total cases
  scale_fill_gradient(low="yellow", high="red") + #set color fill 
  theme_bw() #theme of dark text on light background 
