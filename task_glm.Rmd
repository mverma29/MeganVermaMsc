...
``` {r}
owid <- read_csv("owid_covid.csv") # replace NULL with file name
```
...
epiDisplay::logistic.display(case_glm) # currently broken
...
<!-- Epidisplay still broken but yes, for every 10-fold increase in GDP, there is a 3-fold increase in cases  -->
...
``` {r}
un_data <- readxl::read_xlsx ("un_hh.xlsx", sheet= 4, range= "A5:E819", col_names = TRUE)
...
un_hh_iso <- un_data_filt %>% 
  group_by(`iso_code_l`) %>% 
  summarise (mean_hh= mean(average_hh, na.rm = TRUE))
```
...
un_hh_iso_plot <- merge (x=un_hh_iso, y=world, by.x="iso_code_l", by.y="iso_a3") 
...
un_hh_world <- merge(x=un_hh_iso, y = world) # i don't think all.x is what we want. i'd suggest making a new data frame containing geom, pop, isocode, mean cases.
un_hh_world <- un_hh_world %>% dplyr::select(iso_code_l, mean_hh, un_subregion, pop_est)
# unsure why we have so many obs
