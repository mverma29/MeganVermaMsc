...
# Megan: is the output that for every 10-fold increase in gdp, the total cases per million decreases by 3.4 (percent?)? 
...
``` {r}
glance(gam_full_smooth) 
glance(gam_gdp_smooth)
glance(gam_gini_smooth)
glance(gam_mean_hh_smooth)

# full model has a MUCH lower AIC -- preferred 
```
...
In the `glm()` function we specify an interaction between linear terms as `y ~ x1*x2`. In mgcv's `gam()` function we can specify the interaction between two smooth terms ..


``` {r}
gam_int_smooth <- gam(data    = covid_data,
                       formula = p ~ s(`SI.POV.GINI`, `log_gdp`) + s(`mean_hh`),
                       family  = "binomial", weights = pop_est)
